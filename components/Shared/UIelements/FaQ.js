...
const Faq = ({ children, title, Component, classes, locked = false, initialIsOpen = false, containerClasses = '' }) => {
	const { isOpen, toogle } = UseToogle(initialIsOpen);
	return (
		<div className={`faq ${isOpen && !locked ? 'open' : ''}${containerClasses && containerClasses}`}>
			<div onClick={toogle} className={`faq_header sp-btw flexi `}>
				{title ? <h4 className='form_heading'>{title}</h4> : Component}
				<div>{!locked && <Icon id={'#rarr'} classes='small_svg' />}</div>
			</div>
