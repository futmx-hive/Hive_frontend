...
import StorageService from "services/localStore";
...
class Submission {
  ...
  static headers() {
    const token = t ? t : JSON.parse(localStore. StorageService.getValueFromKey("twix_chunk"));
  ...
  static async submitChapters(body) {
    const url = `${base.url}student/submission/create`;
    return axios.post(
      url,
      {
        ...body,
